...
function resolveReturnEditorUrl() {
  if (!returnToParam) return null;
  try {
    const url = new URL(returnToParam, location.href);
    if (
      url.origin !== location.origin ||
      !url.pathname.endsWith('/admin/equipment-editor.html')
    ) {
      return null;
    }
    return url;
...
const returnEditorUrl = resolveReturnEditorUrl();
const returnEquipmentId = returnEditorUrl?.searchParams.get('id') || null;
...
function normalizeBonus(bonus) {
  ...
  return {
    ...
    stats:
      bonus.stats &&
      typeof bonus.stats === 'object' &&
      !Array.isArray(bonus.stats)
        ? bonus.stats
        : {}
  };
}
...
function normalizeData(source = {}) {
  ...
  return {
    ...
    enabled:
      typeof equipment.enabled === 'boolean'
        ? equipment.enabled
...
async function resolveSetId() {
  ...
  const existingSet = findSet(validatedDraft.setName);
  if (existingSet) {
    return existingSet.id;
  }

  throw new Error(
    `O conjunto "${validatedDraft.setName}" não existe no catálogo. Por segurança ele não será criado automaticamente; selecione ou crie o conjunto manualmente no editor.`
  );
}
...
function buildUpdateDiff(bundle) {
  ...
  const fields = [
    ...
    ['enabled', 'Ativo', equipment.enabled !== false, validatedDraft.enabled === null ? equipment.enabled !== false : Boolean(validatedDraft.enabled)],
...
async function performSave(existing = null, selectedKeys = null) {
  ...
  const saved = await saveEquipmentBundle({
    ...
    equipment: {
      ...
      enabled: take(
        'enabled',
        validatedDraft.enabled === null
          ? current.enabled !== false
          : Boolean(validatedDraft.enabled),
        current.enabled !== false
      ),
    ...
    variants,
    bonuses,
    replaceVariants: updateAll,
    replaceBonuses: updateAll,
    auditSource: 'json-import'
  });
...
async function saveEquipment() {
  ...
  try {
    ...
    if (returnEquipmentId) {
      existingBundle = await getEquipmentBundle(returnEquipmentId);
      existing = existingBundle?.equipment || null;
    } else {
      existing = await getEquipmentBySlug(slug);
      if (existing?.id) {
        existingBundle = await getEquipmentBundle(existing.id);
      }
    }
    ...
    const editorUrl =
      returnEditorUrl
        ? new URL(returnEditorUrl.href)
        : new URL('./equipment-editor.html', location.href);

    editorUrl.searchParams.set('import', '1');
    editorUrl.searchParams.set('source', 'json');
