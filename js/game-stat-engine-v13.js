import * as core from './game-stat-engine-v12-core.js?v=1';
export * from './game-stat-engine-v12-core.js?v=1';
...
export const STAT_DEFINITIONS = {
  ...
  dispersion: { nome: 'Dispersão de tiro', icone: '⌖', cor: '#5b8def', lowerBetter: true },
  moving_dispersion: { nome: 'Dispersão em movimento', icone: '⌖', cor: '#5b8def', lowerBetter: true },
  aimed_dispersion: { nome: 'Dispersão com mira', icone: '⌖', cor: '#5b8def', lowerBetter: true }
};
...
export function resolveEquipmentModifierRule(rawKey) {
  const found=findConcept(rawKey);
  if(found){
    const operation=percentMarked(rawKey)?'percent':found.operation;
    return {
      recognized:true,
  ...
  if(STAT_DEFINITIONS[key]){
    return {recognized:true,canonical:key,target:key,operation:'add',source:'atributo oficial',catalogId:null,confidence:1};
  }

  const pt=core.resolverChavePT(rawKey);
  if(pt){
    return {recognized:true,canonical:key,target:pt.alvo,operation:pt.modo==='percent'?'percent':'add',source:'regra textual legada',catalogId:null,confidence:.8};
  }
...
export function applyEquipmentStats(baseInput={}, equipmentStats=[]) {
  const base=core.normalizeGameStats(baseInput);
  const final={...base}, applied=[], unknown=[];

  for(const source of equipmentStats) for(const [rawKey,rawValue] of Object.entries(source||{})){
...
export function buildRealStatLines(calculation,limit=5) {
  ...
  const changed=Object.keys(calculation.final).filter(k=>k in calculation.base&&STAT_DEFINITIONS[k]&&Math.abs(Number(calculation.final[k])-Number(calculation.base[k]))>1e-9);
  const available=[...new Set([...changed,...priority,...Object.keys(calculation.final)])].filter(k=>k in calculation.base&&STAT_DEFINITIONS[k]);

  return available.slice(0,limit).map(key=>{
    const d=STAT_DEFINITIONS[key], base=Number(calculation.base[key]||0), value=Number(calculation.final[key]||0);
    ...
    return {
      ...
      base,valor:value,difference,delta:rawDelta,beneficialDelta:d.lowerBetter?-rawDelta:rawDelta,pctBase:Math.abs(base)/scale*100,pct:Math.abs(value)/scale*100
    };
  });
}
