-- EchoArena
-- Hardening do fluxo transacional de Builds.
-- Não altera cálculos; valida apenas autoria, acesso e compatibilidade estrutural.
...
begin
  ...
  loop
    ...
    if not exists (
      ...
      where e.id = v_equipment
        and e.enabled = true
        and (
          e.hero_id = p_hero_id
          or (e.class_id is not null and e.class_id = v_hero_class)
          or (e.hero_id is null and e.class_id is null)
        )
    ) then
  ...
  if p_build_id is not null then
    select b.user_id, coalesce(b.is_public, false)
      into v_existing_owner, v_existing_public
    ...
    if found and v_existing_owner = v_user then
      ...
      v_is_update := true;
    elsif found and v_existing_public then
      v_parent_id := p_build_id;
      v_is_fork := true;
    elsif found then
      raise exception 'build_source_not_accessible' using errcode = '42501';
    end if;
  end if;
  ...
  if v_is_update then
    ...
    values (
      v_user, p_hero_id, btrim(p_title),
...
revoke execute on function public.save_user_build(uuid, uuid, text, text, text, text, jsonb, text[])
  from public, anon;
grant execute on function public.save_user_build(uuid, uuid, text, text, text, text, jsonb, text[])
  to authenticated, service_role;

-- Clone explícito: só clona uma build realmente pública ou do próprio usuário/admin.
-- Preserva tier dos itens e tags do original.
create or replace function public.clone_build(source_build_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public, pg_temp
as $$
declare
  source_build public.builds%rowtype;
  new_build_id uuid;
  v_user uuid := auth.uid();
begin
  if v_user is null then
  ...
  insert into public.builds(
    user_id, hero_id, title, description, is_public, visibility, status,
    parent_build_id, published_at, version, updated_at
  )
  values (
    v_user, source_build.hero_id, source_build.title || ' (cópia)',
    source_build.description, false, 'private', 'draft', source_build.id,
    null, 1, now()
  )
  returning id into new_build_id;

  insert into public.build_items(build_id, equipment_id, tier_id, slot)
  select new_build_id, equipment_id, tier_id, slot
  from public.build_items
  where build_id = source_build_id;

  insert into public.build_tag_links(build_id, tag_id)
  select new_build_id, tag_id
  from public.build_tag_links
  where build_id = source_build_id
  on conflict do nothing;
