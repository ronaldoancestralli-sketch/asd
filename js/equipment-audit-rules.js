 ...
 * - efeitos reais sem base oficial podem ser preservados sem bloquear
 *   a estruturação dos demais efeitos calculáveis do mesmo bônus.
 */
...
function aggregateClaims(claims) {
  ...
  for (const claim of claims) {
    if (!claim.structuredKey || claim.externalData) continue;
    aggregate.set(
      claim.structuredKey,
...
const EXTERNAL_STOP = new Set(['a','ao','aos','da','das','de','do','dos','heroi']);
function externalTokens(value = '') {
  return basicNormalize(value)
    .replace(/%/g, ' percentual ')
    .replace(/\bporcentagem\b|\bpct\b/g, ' percentual ')
    .split(/\s+/)
    .filter(token => token && !EXTERNAL_STOP.has(token));
}

function sameExternalMeaning(input, candidate) {
  const a = externalTokens(input);
  const b = externalTokens(candidate);
  if (!a.length || !b.length) return false;
  if (a.length !== b.length) return false;
  return a.every((token, index) => token === b[index]);
}

function resolveExternalClaim(engineKey, externalClassifications = []) {
  const staticEffect = resolveExternalDataEffect(engineKey);
  if (staticEffect) {
    return {
      source: 'catálogo externo conhecido',
      label: staticEffect.label,
      reason: staticEffect.reason || '',
      missingData: staticEffect.missingData || '',
      publicNote: staticEffect.publicNote || ''
    };
  }

  for (const row of externalClassifications || []) {
    if (row?.active === false || row?.classification !== 'external_data_required') continue;
    const candidates = [row.raw_label, row.display_label].filter(Boolean);
    if (!candidates.some(candidate => sameExternalMeaning(engineKey, candidate))) continue;
    return {
      source: 'classificação persistente da Auditoria',
      label: row.display_label || row.raw_label || engineKey,
      reason: row.reason || '',
...
export function buildBonusMigrationAudit(rows = [], setsById = new Map(), externalClassifications = []) {
  ...
  for (const group of groups.values()) {
    ...
    for (const row of group) {
      ...
      claims.forEach(claim => {
        claim.externalData = resolveExternalClaim(claim.engineKey, externalClassifications);
      });
      const expectedClaims = numericClaimCount(row.description || '');
      const currentSnapshot = aggregateClaims(claims);
      const externalClaims = claims.filter(claim => claim.externalData);
      ...
      for (const claim of claims) {
        if (claim.externalData) continue;
      ...
      } else if (item.mode !== 'external-data') {
        item.severity = 'blocker';
      }
